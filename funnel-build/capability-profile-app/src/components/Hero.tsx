...
function HeroInstrument() {
  return (
    <svg className="hero-instrument" viewBox="0 0 660 420" role="img" aria-labelledby="hero-visual-title hero-visual-desc">
      ...
      <desc id="hero-visual-desc">A wire passes through preheating, extrusion, diameter measurement, spark testing and tension control stages, with an offline testing branch.</desc>
      <rect x="20" y="40" width="620" height="320" rx="8" className="instrument-frame" />
      <path d="M62 214H598" className="wire-path" />
      <path d="M408 214V318H548" className="offline-path" />
      ...
      <text x="44" y="82" className="diagram-label">CAPABILITY PROFILE / REVIEW BUILD</text><text x="44" y="338" className="diagram-note">*Across approved product series; model-specific limits apply.</text>
    </svg>
  );
}
...
export function Hero() {
  return (
    <section className="hero" id="top">
      ...
      <div className="container proof-strip" aria-label="Puretronics capability facts">{['Established in 1990', '5 Wire and Cable Product Families', '13 Primary Products', 'Inline, offline and process-control equipment'].map((item, index) => <div key={item}><span>0{index + 1}</span><strong>{item}</strong></div>)}</div>
