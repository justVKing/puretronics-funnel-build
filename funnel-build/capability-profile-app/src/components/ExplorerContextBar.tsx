...
export function ExplorerContextBar() {
  ...
  return <div className="explorer-context-bar" aria-label="Active Filters Across Explorer Views"><strong>Your Active Filters</strong><div>{state.filters.query && <button type="button" onClick={() => dispatch({ type: 'SET_QUERY', query: '' })}>Search: {state.filters.query} ×</button>}{chips.map((chip) => <button type="button" key={`${chip.key}-${chip.id}`} onClick={() => dispatch({ type: 'TOGGLE_FILTER', key: chip.key, value: chip.id })}>{chip.label} ×</button>)}</div><button type="button" className="text-button" onClick={() => dispatch({ type: 'CLEAR_FILTERS' })}>Clear Filters</button></div>;
}
