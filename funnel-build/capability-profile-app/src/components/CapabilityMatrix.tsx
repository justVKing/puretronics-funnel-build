...
const relationshipLabels = {
  primary: 'Primary V4 Placement', adjacent: 'Adjacent Process Context', supporting: 'Supporting Role', 'downstream-response': 'Downstream Response Context',
} as const;
...
export function CapabilityMatrix() {
  ...
  const rows = useMemo(() => requirementDefinitions.map((requirement) => {
    ...
    const eligibleIds = new Set(matchProducts({ ...state.filters, problems: [], query: labelMatch ? '' : requirementQuery }).map((result) => result.productId));
  ...
  return <div className="matrix-view">
    ...
    {requirementQuery && <div className="active-filter-bar" aria-label="Matrix Search"><strong>Matrix Search</strong><button type="button" onClick={() => setRequirementQuery('')}>Search: {requirementQuery} ×</button><button type="button" className="text-button" onClick={() => setRequirementQuery('')}>Clear Search</button></div>}
