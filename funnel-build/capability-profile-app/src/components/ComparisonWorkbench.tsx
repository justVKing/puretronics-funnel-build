...
export function ComparisonWorkbench() {
  ...
  const [comparisonLevel, setComparisonLevel] = useState<'models' | 'skus'>('models');
  ...
  const skuCandidates = product?.models.flatMap((model) => model.children ?? []) ?? [];
  const candidates = comparisonLevel === 'skus' ? skuCandidates : product?.models ?? [];
  const candidateKey = candidates.map((model) => model.id).join('|');
  ...
  useEffect(() => {
    ...
    candidates.slice(0, 2).forEach((model) => dispatch({ type: 'TOGGLE_COMPARISON_MODEL', modelId: model.id, limit: 3 }));
  }, [candidateKey, candidates, dispatch, product, state.comparisonModelIds.length]);
  ...
  const visibleTechnicalRowCount = visibleSections.find((section) => section.id === 'technical')?.rows.length ?? 0;

  const chooseProduct = (productId: string) => {
    setComparisonLevel('models');
    dispatch({ type: 'SET_COMPARISON_PRODUCT', productId: productId || null });
    track('comparison_viewed', { mode: 'models', ids: productId ? [productId] : [] });
  };

  const recordCountLabel = (item: typeof comparableProducts[number]) => {
    const skuCount = item.models.flatMap((model) => model.children ?? []).length;
    return skuCount ? `${item.models.length} series · ${skuCount} capacity SKUs` : `${item.models.length} models / variants`;
  };
  ...
  return (
    <div className="comparison-view">
      ...
      <section className="comparison-setup" aria-labelledby="comparison-product-heading">
        <div className="comparison-step"><span>01</span><div><h4 id="comparison-product-heading">Choose a Primary Product</h4><p>Only products with two or more governed models, variants or series are listed.</p></div></div>
        <label className="comparison-product-select"><span>Primary Product</span><select value={product?.id ?? ''} onChange={(event) => chooseProduct(event.target.value)}><option value="">Select a Product to Compare</option>{comparableProducts.map((item) => <option value={item.id} key={item.id}>{item.id} · {item.name} ({recordCountLabel(item)})</option>)}</select></label>
      </section>
      ...
      {product && <>
        ...
        <section className="model-selector" aria-labelledby="model-selector-heading"><div className="comparison-step"><span>02</span><div><h4 id="model-selector-heading">Choose Models or Variants</h4><p>Select up to {selectionLimit} records on this screen. The complete V4 technical table updates immediately.</p></div></div>
          {skuCandidates.length > 0 && <fieldset className="comparison-level"><legend>Comparison Record Type</legend><label className={comparisonLevel === 'models' ? 'is-selected' : ''}><input type="radio" name="comparison-level" value="models" checked={comparisonLevel === 'models'} onChange={() => { setComparisonLevel('models'); dispatch({ type: 'SET_COMPARISON_PRODUCT', productId: product.id }); }} /><span>Loadcell Series</span></label><label className={comparisonLevel === 'skus' ? 'is-selected' : ''}><input type="radio" name="comparison-level" value="skus" checked={comparisonLevel === 'skus'} onChange={() => { setComparisonLevel('skus'); dispatch({ type: 'SET_COMPARISON_PRODUCT', productId: product.id }); }} /><span>Capacity SKUs</span></label></fieldset>}
          <div className="model-choice-grid">{candidates.map((model) => { const checked = state.comparisonModelIds.includes(model.id); const disabled = !checked && state.comparisonModelIds.length >= selectionLimit; return <label className={checked ? 'is-selected' : ''} key={model.id}><input type="checkbox" checked={checked} disabled={disabled} onChange={() => dispatch({ type: 'TOGGLE_COMPARISON_MODEL', modelId: model.id, limit: selectionLimit })} /><span>{model.id}</span><strong>{model.name}</strong><small>{model.specs.length} V4 Technical Parameter{model.specs.length === 1 ? '' : 's'}</small></label>; })}</div><p className="selection-note" aria-live="polite">{state.comparisonModelIds.length} model or variant record{state.comparisonModelIds.length === 1 ? '' : 's'} selected{isMobile && state.comparisonModelIds.length > 2 ? '; the first two are shown on mobile' : ''}.</p></section>

        {visibleModels.length > 0 && <section className="comparison-output" aria-labelledby="comparison-output-heading"><div className="comparison-output-header"><div className="comparison-step"><span>03</span><div><h4 id="comparison-output-heading">Review All Approved Specifications</h4><p>{visibleTechnicalRowCount} technical parameter{visibleTechnicalRowCount === 1 ? '' : 's'}, plus publication context, shown across {visibleModels.length} selected model{visibleModels.length === 1 ? '' : 's'}.</p></div></div><label className="check-control"><input type="checkbox" checked={state.comparisonShowDifferences} onChange={(event) => dispatch({ type: 'SET_COMPARISON_DIFFERENCES', show: event.target.checked })} /><span>Show Differences Only</span></label></div>
